...
<schema fpi="http://schemas.opengis.net/om/2.0/coverageObservation.sch"
    ...
    <pattern id="CoverageTest">
        <rule context="//om:OM_Observation/om:result">
            <assert
                test="schema-element(gml:AbstractDiscreteCoverage)|schema-element(cv:CV_AbstractDiscreteCoverage)"
                >result must contain an element in the substitution group headed by
                cv:CV_AbstractDiscreteCoverage or gml:AbstractDiscreteCoverage</assert>
        </rule>
    </pattern>
