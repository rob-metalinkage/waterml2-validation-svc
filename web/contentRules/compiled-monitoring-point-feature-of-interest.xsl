...
<xsl:template match="/"><svrl:schematron-output xmlns:svrl="http://purl.oclc.org/dsdl/svrl" title="OGC WaterML2.0 observation validation" schemaVersion=""><xsl:comment><xsl:value-of select="$archiveDirParameter"/>   
		 ...
		 <xsl:value-of select="$fileDirParameter"/></xsl:comment><svrl:text>Verifies that the om:featureOfInterest element contains a OGC WaterML2.0 monitoring point type.</svrl:text><svrl:ns-prefix-in-attribute-values uri="http://www.opengis.net/waterml/2.0" prefix="wml2"/><svrl:ns-prefix-in-attribute-values uri="http://www.opengis.net/om/2.0" prefix="om"/><svrl:ns-prefix-in-attribute-values uri="http://www.opengis.net/gml/3.2" prefix="gml"/><svrl:ns-prefix-in-attribute-values uri="http://www.w3.org/1999/xlink" prefix="xlink"/><svrl:active-pattern><xsl:attribute name="document"><xsl:value-of select="document-uri(/)"/></xsl:attribute><xsl:attribute name="id">featureOfInterest</xsl:attribute><xsl:attribute name="name">Test requirement: /req/xsd-feature-of-interest-monitoring-point/featureOfInterest</xsl:attribute><xsl:apply-templates/></svrl:active-pattern><xsl:apply-templates select="/" mode="M8"/></svrl:schematron-output></xsl:template>

<!--SCHEMATRON PATTERNS-->
<svrl:text xmlns:svrl="http://purl.oclc.org/dsdl/svrl">OGC WaterML2.0 observation validation</svrl:text>

<!--PATTERN featureOfInterestTest requirement: /req/xsd-feature-of-interest-monitoring-point/featureOfInterest-->
<svrl:text xmlns:svrl="http://purl.oclc.org/dsdl/svrl">Test requirement: /req/xsd-feature-of-interest-monitoring-point/featureOfInterest</svrl:text>

	<!--RULE -->
<xsl:template match="//om:OM_Observation/om:featureOfInterest" priority="1000" mode="M8"><svrl:fired-rule xmlns:svrl="http://purl.oclc.org/dsdl/svrl" context="//om:OM_Observation/om:featureOfInterest"/>

		<!--ASSERT -->
<xsl:choose><xsl:when test="schema-element(wml2:MonitoringPoint) | @xlink:href"/><xsl:otherwise><svrl:failed-assert xmlns:svrl="http://purl.oclc.org/dsdl/svrl" test="schema-element(wml2:MonitoringPoint) | @xlink:href"><xsl:attribute name="location"><xsl:apply-templates select="." mode="schematron-select-full-path"/></xsl:attribute><svrl:text>The xml element om:featureOfInterest shall contain a subelement of wml2:MonitoringPoint, a
        member of its substitution group or a reference to a representation of the monitoring point using xlink.</svrl:text></svrl:failed-assert></xsl:otherwise></xsl:choose><xsl:apply-templates select="*|comment()|processing-instruction()" mode="M8"/></xsl:template><xsl:template match="text()" priority="-1" mode="M8"/><xsl:template match="@*|node()" priority="-2" mode="M8"><xsl:apply-templates select="*|comment()|processing-instruction()" mode="M8"/></xsl:template></xsl:stylesheet>